...
应用在 multipart 输入流上逐块计数，不预先拼接完整请求体；原生
`Request.formData()` 仍会保留解析后的 `File`，PDF 校验随后会执行一次
`File.arrayBuffer()`。因此 25 MiB 上限仍是明确的单请求内存边界，而不是流式直写磁盘；
公网部署还必须由 Nginx 或受信任上游补充 rate/connection limit。
