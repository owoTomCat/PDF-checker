...
该脚本验证 commit、release 路径、worker 产物 `dist/audit-worker.mjs` 以及 web 产物
`dist/server/index.js`、`dist/server/vinext-server.json`，并用同父目录临时
...
这组命令只用于首次把普通 `current` 目录迁移成 symlink 后的失败恢复。所有断言必须
成功；尤其只能在 `test -L` 成功且唯一真实备份目录已经确认后删除 symlink。
...
Nginx 监听 IPv4/IPv6 80 端口并代理到 web 服务，`client_max_body_size 25m` 保持不变。上传接口使用 `application/pdf` 原始请求体；UTF-8 文件名可逆 percent 编码后只放在有界的 `X-Pdf-File-Name` ASCII 头中，不进入 URL 或默认 combined access log，服务端校验解码后仅写入 SQLite。接口只在私有文件和 SQLite 排队记录都成功写入后返回。模型处理由独立 worker 完成，所以浏览器断开不会中断任务。

原始 PDF 不会触发 vinext 的 multipart/Server Action 预解析。应用有界读取器逐块计数，
最多同时保留输入 chunks 和一份连续 `Uint8Array`，峰值约 2 倍 20 MiB payload 加运行时
开销，然后校验 `%PDF-` 并写入 UUID 命名的私有文件；这不是流式直写磁盘。Nginx 的
上传 key 只匹配 `POST /api/tasks`，同一来源最多 3 个并发上传，速率为 `6r/s` 且允许
`burst=12`；GET 轮询和其他接口使用空 key，不计入该限制。
