...
Nginx 监听 IPv4/IPv6 80 端口并代理到 web 服务，`client_max_body_size 25m` 保持不变。上传接口使用 `application/pdf` 原始请求体，文件名放在 URL 编码且有界的 `fileName` 查询参数中；接口只在私有文件和 SQLite 排队记录都成功写入后返回。模型处理由独立 worker 完成，所以浏览器断开不会中断任务。
